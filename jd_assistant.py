...
class Assistant(object):
    ...
    def get_single_item_stock(self, sku_id, num, area):
        ...
        resp_text = ''
        try:
            # rrre = requests.get('https://c0.3.cn/stocks?type=getstocks&skuIds=100011540165&area=3_51043_2907').text
            resp_text = requests.get(url=url, params=payload, headers=headers, timeout=self.timeout).text
            # resp_text = requests.get('https://c0.3.cn / stocks?type = getstocks & skuIds = 100011540165 & area = 3_51043_2907').text
            resp_json = parse_json(resp_text)

            # stock_info = resp_json.get('stock')
            # sku_state = resp_json.get('code')  # 商品是否上架
            # stock_state = resp_json.get('StockState')  # 商品库存状态：33 -- 现货  0,34 -- 无货  36 -- 采购中  40 -- 可配货
            # return sku_state == 1 and stock_state in (33, 40)

            temp = resp_json.get('stock')
            stock_state = temp.get('StockState')
